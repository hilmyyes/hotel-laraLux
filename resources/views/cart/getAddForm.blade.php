<form method="POST" action="{{ route('cart.store') }}">
    ...
    <input type="number" class="form-control" id="Price" value="{{ $data->price }}" disabled><br>

    <label for="CheckIn">Check-In Date</label>
    <input type="date" class="form-control" name="CheckIn" id="CheckIn" value="{{ date('Y-m-d', strtotime('now')) }}" min="{{ date('Y-m-d', strtotime('now')) }}"
        onchange="UpdateCheckOut()"><br>

    <label for="Duration">Duration</label>
    <input type="number" class="form-control" name="Duration" id="Duration" onchange="UpdateCheckOut()"
        placeholder="Enter Stay Duration" value="1" min="1" max="30"><br>

    <label>CheckOut: </label><label id="CheckOut"></label><br><br>

    <label for="SubTotal">Sub Total</label>
    <input type="number" class="form-control" name="SubTotal" id="SubTotal" value="{{ $data->price }}" readonly><br>
